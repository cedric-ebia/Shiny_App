...
df_all = data.frame(readRDS("df.rds"))
attach(df_all)
```
...
<center>
![Because a picture is worth a thousand words!](split.PNG)
</center>
...
```{r comparison_ROC1, echo=FALSE, fig.align ="center" , results="asis", message=FALSE,warning=FALSE}
...
```{r comparison_ROC, echo=FALSE, fig.align ="center"}
ggplot() +
  # green plot
...
```{r comparison_PRC, echo=FALSE, fig.align ="center"}
    ggplot() +
      # green plot
...
## Difference between the RUS training set and the training set

We need to keep in mind that we trained all of our models on the RUS training set, which is about $0.01%$ of the dataset, for computation time sake. However, XGBoost and Logistic Regression handle pretty well bigger sets of data. If we had trained XGBoost and the Logistic Regression on the whole training set, the results are way better in favor of XGBoost and Logistic Regression ! For the others models (SVM and KNN), having them trained on the whole training set doesn't change the results that much.

### Results for the XGBoost trained on the whole training set

```{r xgb_whole_dataset, echo=FALSE, fig.align ="center", message=FALSE, warning=FALSE}
train_xgb_all = train
test_xgb_all = test
train_xgb_all$Class = as.numeric(as.character(train_xgb_all$Class))
xgb.data.train <- xgb.DMatrix(as.matrix(train_xgb_all[, colnames(train_xgb_all) != "Class"]), label = train_xgb_all$Class)
xgb.data.test <- xgb.DMatrix(as.matrix(test_xgb_all[, colnames(test_xgb_all) != "Class"]), label = test_xgb_all$Class)

test_xgb_all$Class = as.integer(test_xgb_all$Class)

test_xgb_all$Class[test_xgb_all$Class == 1] = 0
test_xgb_all$Class[test_xgb_all$Class == 2] = 1
...
test_xgb_all$predicted= predict(xgb.model
                    , newdata = as.matrix(test_xgb_all[, colnames(test_xgb_all) != "Class"])
                    , ntreelimit = xgb.model$bestInd)
```


```{r confusion_matrix_xgboost_all, echo=FALSE, fig.align ="center"}
plot_confusion_matrix(test_xgb_all, "XGBoost using AUCPR as evaluation metric")
```

```{r roc_auc_xgboost_all, include=FALSE, fig.align ="center", results="asis",message = FALSE, warning=FALSE}
set.seed(1337)
test_xgb_plot = df[train.test.split == 2,]
predictions_xgb= predict(xgb.model
                         , newdata = as.matrix(test_xgb_plot[, colnames(test_xgb_plot) != "Class"])
...
```
```{r xgb_2, echo=FALSE, fig.align ="center", results="asis", message=FALSE,warning=FALSE}
autoplot(xgb_performance)
```

### Results for the Logistic Regression trained on the whole training set

```{r glm_all, echo=FALSE, fig.align ="center", results="asis", message=FALSE,warning=FALSE}
set.seed(1337)
...
```{r confusion_matrix_glm_all, echo=FALSE, fig.align ="center"}
plot_confusion_matrix(test_glm_plot,"Logistic Regression")
```


```{r glm_plot_all, echo=FALSE, fig.align ="center", results="asis", message=FALSE,warning=FALSE}
autoplot(logreg_performance)
```

## Model Comparison for the models trained on the whole training set


```{r comparison_ROC1_all, include=FALSE, fig.align ="center", results="asis",message = FALSE, warning=FALSE, echo=FALSE}
    predictions_svm2 <- predict(classifier,newdata = test_svm_plot, probability=T)
    svm2_predict_obj <- mmdata(as.numeric(predictions_svm2),test_svm_plot$Class)
    svm2_performance <- evalmod(svm2_predict_obj)
...
```{r comparison_ROC_all, echo=FALSE, fig.align ="center"}
ggplot() +
  # green plot
  geom_line(data=roc, aes(x=x, y=y,colour=classifier)) + 
  geom_abline(intercept=0, slope=1, linetype = "dashed") +
...
```{r comparison_PRC_all, echo=FALSE, fig.align ="center"}
    ggplot() +
      # green plot
      geom_line(data=prc, aes(x=x, y=y,colour=classifier)) + 
      ylab("Precision") +
